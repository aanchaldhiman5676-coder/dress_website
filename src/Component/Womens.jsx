...
const Womens = () => {
  const image = Object.values(picture);
  const [counts, setCounts] = useState(Array(image.length).fill(0));
  const [wishlist, setWishlist] = useState(Array(image.length).fill(false));
  ...
  const increment = (index) => {
    ...
    newCounts[index]++;
    setCounts(newCounts);
  addToCart(`womens-${index}`, image[index]); 
  };
    ...
    {image.map((img, index) => (
      <div key={index} className='bg-white rounded-xl shadow-lg overflow-hidden w-full max-w-[280px] hover:scale-105 transition-transform duration-300'>
        ...
        <div className='p-4 flex flex-col items-center'>
          ...
          <button onClick={() => toggleWishlist(index)} className='flex items-center gap-2 text-red-500 hover:text-red-600 transition'>
